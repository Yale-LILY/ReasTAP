...
os.environ["WANDB_PROJECT"] = "reastap_tabfact_march"
...
def main():
    ...
    if training_args.do_eval:
        ...
        eval_dataset = raw_datasets[inference_set]
        if data_args.max_eval_samples is not None:
            eval_dataset = eval_dataset.select(range(data_args.max_eval_samples))

    if training_args.do_predict:
        inference_set = data_args.inference_set
        predict_dataset = raw_datasets[inference_set]
        raw_predict_dataset = raw_datasets[inference_set]
    ...
    def compute_metrics_for_prediction(p: EvalPrediction):
        preds = p.predictions[0] if isinstance(p.predictions, tuple) else p.predictions
        preds = np.argmax(preds, axis=1)
        return {"accuracy": (preds == p.label_ids).astype(np.float32).mean().item()}
    # You can define your custom compute_metrics function. It takes an `EvalPrediction` object (a namedtuple with a
    # predictions and label_ids field) and has to return a dictionary string to float.
    def compute_metrics_for_train(p: EvalPrediction):
        preds = p.predictions[0] if isinstance(p.predictions, tuple) else p.predictions
        # preds = np.argmax(preds, axis=1)
        return {"accuracy": (preds == p.label_ids).astype(np.float32).mean().item()}
    ...
    trainer = Trainer(
        ...
        compute_metrics=compute_metrics_for_prediction if training_args.do_predict else compute_metrics_for_train,
        tokenizer=tokenizer,
        data_collator=data_collator,
        preprocess_logits_for_metrics=preprocess_logits_for_metrics if not training_args.do_predict else None,
    )
    ...
    if training_args.do_predict:
        ...
        predictions = trainer.predict(predict_dataset, metric_key_prefix="predict").predictions[0]
        ...
        for i, pred in enumerate(predictions):
            ...
            example_id = str(i)
            outputs.append({"id": example_id, "prediction": int(pred), "answers": answer})
            if pred == answer:
                correct += 1
            total += 1
